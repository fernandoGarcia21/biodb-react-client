...
export function CreateTraitForm(): React.JSX.Element {
      ...
      useEffect(() => {
          const fetchTraitTypes = async () => {
            try {
              ...
              console.error('Error fetching trait type info:', error);
            }
          };
  ...
  const onSubmit = React.useCallback(
      async (values: Values): Promise<void> => {
        ...
        try{
          await createTraitRequest(values);
          reset({ name: "", description: "", trait_type_id: 0 });
          setSuccessMessage(`Trait ${values.name} created successfully!`);
          setIsPending(false);
