...
export function UpdateLocationForm(): React.JSX.Element {
...
//When the Update button is pressed, the form is submitted and the location is updated
  const onSubmit = React.useCallback(
      async (values: Values): Promise<void> => {
        setIsPending(true);
        setSuccessMessage(null);
        try{
          await updateLocationRequest(locationId, values);
          setSuccessMessage(`Location ${values.name} updated successfully!`);
          setIsPending(false);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Card>
        <CardHeader title={`Update location ${locationName}`} />
        <Divider />
        <CardContent>
